...
export class LiabilityService {
  ...
  async createLiability(liabilityData: Partial<Liability>): Promise<Liability> {
    ...
    // Normalize as_of_month (accepts 'YYYY-MM' or full date)
    let normalizedAsOfMonth: string | Date | undefined = as_of_month as any;
    if (typeof normalizedAsOfMonth === 'string' && /^\d{4}-\d{2}$/.test(normalizedAsOfMonth)) {
      normalizedAsOfMonth = `${normalizedAsOfMonth}-01`;
    }
  ...
  async updateLiability(id: string, updateData: Partial<Liability>): Promise<Liability | null> {
    ...
    Object.entries(updateData).forEach(([key, value]) => {
      if (value !== undefined) {
        let normalizedValue = value as any;
        if (key === 'as_of_month' && typeof normalizedValue === 'string' && /^\d{4}-\d{2}$/.test(normalizedValue)) {
          normalizedValue = `${normalizedValue}-01`;
        }
